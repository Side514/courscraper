...
def get_reviews(course_ID: str) -> list:
    ...
    def request():
        ...
        # 发送请求
        print(f"Requesting... ({start} - {start+999})")
        response = requests.post(api, params=params, json=payload, headers=header)
        response.encoding = response.apparent_encoding
        return json.loads(response.text)[0]["data"]["ProductReviewsV1Resource"]["reviews"]["elements"]
    ...
    while True:
        ...
        for item in raw_data:
            ...
            text = text.replace("\u200b", "").replace("<text>", "").replace("<text />", "")
            text = re.split(r"\.|!|\?|</text>", text)
            text = [s.strip() for s in text]
            text = list(filter(lambda s: s != "", text))
